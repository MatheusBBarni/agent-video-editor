...
Stream-copies when every input probes and codec, coded size, fps, and `rotate_deg` match. Any missing or unreadable input re-encodes. Mixed rotation (e.g. 0° + 90°) re-encodes so ffmpeg can autorotate.
